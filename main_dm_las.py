...
import PSORPInversion_Phi_v2 as pspi 
import lasio.examples

#semente fixada


model_labels = {1:'Wyllie', 2:'Reymer', 3:'Soft sand', 4:'Stiff sand', 5:'Spherical inclusion', 6:'Berryman inclusion'}
model_colors = {'Wyllie': 'orange', 'Reymer': 'gold', 'Soft sand':'magenta', 'Stiff sand':'lime' , 'Spherical inclusion':'red', 'Berryman inclusion':'darkviolet'}
...
data = dict()
#Arquivo qualquer .las
data["Depth"]  = brute[:,1].reshape(-1, 1)
data["Phi"]    = brute[:,2].reshape(-1, 1)
data["Vp"]     = brute[:,3].reshape(-1, 1)
print(data)
...
#fixing parameters 
fixed = dict()
fixed["Kmat"] = np.nanmean(pK_OFICIAL) #36
fixed["Gmat"] = np.nanmean(pMU_OFICIAL)#45
fixed["RHOmat"]      = np.nanmean(pRHO)#2.65
fixed["Kfl"]         = np.nanmean(pKFLUIDFINAL_AGUA80_QUARTZO)#2.25
fixed["Gfl"]         = np.nanmean(pMU_AGUA80_QUARTZO)
fixed["RHOfl"]       = np.nanmean(pRHO_AGUA80_QUARTZO)
fixed["coordnum"]    = 7
fixed["criticalPhi"] = np.nanmax(aPhi)
fixed["pressure"]    = np.nanmean(pPR_OFICIAL)
...
pso = pspi.PSORPInversion_Phi(data, fixed, 0)#, horizon)
...
data = pso.data["Vp"].copy()
depth = pso.data["Depth"].copy()
phi = pso.data["Phi"].copy()
...
for d in range(len(data)):
    best = None
    all_swarm = []
    pso.data["Vp"] = data[d:d+1]   
    #calculating best PHI error
    temp_swarm_best_phi = {"error": math.inf, "x":None, "VP":None, "M":None}
    
    for m in range(1, 6+1):
        pso.typeModelIt = m #fixa o modelo a ser utilizado
        swarm = pso.optimize(particles, len(pso.data["Vp"]), iterations, w, c1, c2, \
        ...
        for xi in swarm[1]:
            if  temp_swarm_best_phi["error"] > np.square(xi.x[0] - phi[d]):
                ...
                temp_swarm_best_phi["VP"] 	 = xi.VP[0]
    ...
    for ind in range(len(all_swarm)):
        ...
        SWARM[ind]["VP"].append(all_swarm[ind].VP[0]) 
...
for ind in range(individuals):
    for d in range(len(data)-1):
        m = SWARM[ind]["m"][d]
        plt.plot([SWARM[ind]["x"][d], SWARM[ind]["x"][d+1]], [pso.data["Depth"][d], pso.data["Depth"][d+1]], color=model_colors[model_labels[m]], linewidth=2.0, label=model_labels[m])   
...
for m in range(1, 6+1):
    for ind in range(particles+1):
        for d in range(len(data)-1):
            m = SWARM[ind]["m"][d]
            plt.plot([SWARM[ind]["VP"][d], SWARM[ind]["VP"][d+1]], [pso.data["Depth"][d], pso.data["Depth"][d+1]], color=model_colors[model_labels[m]], linewidth=2.0, label=model_labels[m])       
ind = minima["ind"]
